...
Handles environments with or without PyTorch gracefully
"""
import numpy as np
import pandas as pd
...
try:
    ...
    print("✓ PyTorch available - LSTM and GNN models enabled")
except ImportError as e:
    print("⚠️ PyTorch not available or corrupted - Only XGBoost models will be available")
    print(f"   Error: {e}")
...
# PyTorch-dependent models (only defined if PyTorch is available)
if TORCH_AVAILABLE:
    ...
    class LSTMCrisisPredictor:
        def __init__(self, *args, **kwargs):
            raise RuntimeError("PyTorch is not available. Cannot create LSTM model. Please install PyTorch or use XGBoost-only mode.")
    
    class GraphNeuralNetwork:
        def __init__(self, *args, **kwargs):
            raise RuntimeError("PyTorch is not available. Cannot create GNN model. Please install PyTorch or use XGBoost-only mode.")


class XGBoostCrisisModel:
    """XGBoost model for crisis prediction - Works without PyTorch"""
...
class EnsembleCrisisPredictor:
    """Ensemble model combining LSTM, GNN, and XGBoost - Works with or without PyTorch"""
    ...
    def _predict_lstm(self, features) -> np.ndarray:
        """Get LSTM predictions"""
        if not TORCH_AVAILABLE:
            raise RuntimeError("PyTorch not available for LSTM predictions")
        self.models['lstm'].eval()
        with torch.no_grad():
            _, probs = self.models['lstm'](features)
        return probs.cpu().numpy()
    
    def _predict_gnn(self, features: Tuple) -> np.ndarray:
        """Get GNN predictions"""
        if not TORCH_AVAILABLE:
            raise RuntimeError("PyTorch not available for GNN predictions")
        x, adj = features
    ...
    def load_models(self, path: str, input_sizes: Dict):
        ...
        # Load LSTM if PyTorch is available and file exists
        if TORCH_AVAILABLE and os.path.exists(f"{path}/lstm_model.pt"):
            if 'lstm' in input_sizes:
                try:
                    ...
                    print("Warning: Could not load LSTM model")
            
        # Load GNN if PyTorch is available and file exists
        if TORCH_AVAILABLE and os.path.exists(f"{path}/gnn_model.pt"):
            if 'gnn' in input_sizes:
                try:
                    ...
                    print("Warning: Could not load GNN model")
            
        # Load XGBoost
        if os.path.exists(f"{path}/xgboost_model.pkl"):
...
class CrisisModelTrainer:
    """Trainer class for all crisis prediction models - Only instantiated when needed"""
    
    def __init__(self, config: Dict):
        if not TORCH_AVAILABLE:
            raise RuntimeError("CrisisModelTrainer requires PyTorch. Please install PyTorch to train models.")
